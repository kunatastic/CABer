...
<html>
	...
	<body>
        ...
        <?php
            $sql = "select travel_date,location from flight where Username = '$user';";
            ...
            if ($result_check1>0){
                ...
                while ($row1 = mysqli_fetch_assoc($query)) {
                    ...
                    $location = $row1["location"];
                        echo "<hr><h4>Your Requests </h4><hr> <strong> Date:- $date, Location:- $location <br/></strong>";
                    $sql1 = "select Username from flight where travel_date='$date' and location='$location' and not Username = '$user';";
                        ...
                        while ($row2 = mysqli_fetch_assoc($query1)) {
                            ...
                            if ($result_check3>0){
                                while ($row3 = mysqli_fetch_assoc($query2)){
                                    echo "Name:- ".$row3["Name"]."<br>Phone:- ".$row3["phone_no"]."<br>Email ID:- ".$row3["email_id"]."<br/>";
                                };
                            }else{
